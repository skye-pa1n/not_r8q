					...
					int *level_key, bool is_hbm, bool is_hmt)
{
	...
	if (cur_rr == 60) {
		...
		vrr_cmds->cmds[0].msg.tx_buf[1] = 0x09; /* 133 HZ */
	}
...
#define FRAME_WAIT_120FPS (15)		/* HBM   :  9 => 17 */
#define NORMAL_HBM_DELAY_60FPS (6)	/* HBM   : 16 => 6  */
#define NORMAL_HBM_DELAY_120FPS (7)	/* HBM   :  9 => 8  */
#define HBM_NORMAL_DELAY_60FPS (8)	/* NORMAL: 16 => 8  */
#define HBM_NORMAL_DELAY_120FPS (7)	/* NORMAL:  9 => 8  */
							...
							(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	if (ss_panel_revision(vdd) >= 'I') {
		...
		*/
		...
		pcmds->cmds[10].msg.tx_buf[1] = normal_aor_manual[cd_index].aor_63h_132;
		pcmds->cmds[10].msg.tx_buf[2] = normal_aor_manual[cd_index].aor_63h_133;
		pcmds->cmds[10].msg.tx_buf[3] = normal_aor_manual[cd_index].aor_63h_134;
	}
		...
		pcmds->cmds[5].msg.tx_buf[1] = normal_aor_manual[cd_index].aor_63h_132;
		pcmds->cmds[5].msg.tx_buf[2] = normal_aor_manual[cd_index].aor_63h_133;
		pcmds->cmds[5].msg.tx_buf[3] = normal_aor_manual[cd_index].aor_63h_134;
	} else {
		finger_mask_update_delay = vdd->vrr.cur_refresh_rate > 60 ? 9000 : 17000;
		if (pcmds->cmds[6].last_command == 1)
			usleep_range(finger_mask_update_delay, finger_mask_update_delay + 1);
							...
							(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	if (ss_panel_revision(vdd) >= 'I') {
		...
		pcmds->cmds[10].msg.tx_buf[1] = hbm_aor_manual[cd_index].aor_63h_132;
		pcmds->cmds[10].msg.tx_buf[2] = hbm_aor_manual[cd_index].aor_63h_133;
		pcmds->cmds[10].msg.tx_buf[3] = hbm_aor_manual[cd_index].aor_63h_134;
	} else if (ss_panel_revision(vdd) >= 'G') {
		pcmds->cmds[2].msg.tx_buf[1] = get_bit(vdd->br_info.common_br.gm2_wrdisbv, 8, 8);
		pcmds->cmds[2].msg.tx_buf[2] = get_bit(vdd->br_info.common_br.gm2_wrdisbv, 0, 8);
		...
		pcmds->cmds[5].msg.tx_buf[1] = hbm_aor_manual[cd_index].aor_63h_132;
		pcmds->cmds[5].msg.tx_buf[2] = hbm_aor_manual[cd_index].aor_63h_133;
		pcmds->cmds[5].msg.tx_buf[3] = hbm_aor_manual[cd_index].aor_63h_134;
	} else {
		pcmds->cmds[5].msg.tx_buf[1] = get_bit(vdd->br_info.common_br.gm2_wrdisbv, 8, 8);
		pcmds->cmds[5].msg.tx_buf[2] = get_bit(vdd->br_info.common_br.gm2_wrdisbv, 0, 8);
...
static int ss_vrr_init(struct vrr_info *vrr)
{
	...
	vrr->cur_refresh_rate = vrr->adjusted_refresh_rate = 133;
	vrr->cur_sot_hs_mode = vrr->adjusted_sot_hs_mode = true;
	vrr->max_h_active_support_120hs = 1080; /* supports 133hz until FHD 1080 */
...
static void samsung_panel_init(struct samsung_display_driver_data *vdd)
{
	...
	/* te high -> 47us (133fps) -> te low -> tx start */
	vdd->panel_hbm_entry_after_te = 55;
