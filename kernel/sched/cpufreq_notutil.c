 ...
 * @sg_policy: notutil policy object to compute the new frequency for.
...
static struct cpufreq_governor notutil_gov;
...
{
	return cpufreq_register_governor(&notutil_gov);
}
...
{
	if (old_gov == &notutil_gov || policy->governor == &notutil_gov) {
		/*
